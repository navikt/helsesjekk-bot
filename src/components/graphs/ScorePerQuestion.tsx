...
function OverallScoreGraph({
  ...
  const questionsMap = useMemo(
    () => R.indexBy(questions, (it) => it.id),
    [questions]
  );
  const CustomTooltip = useMemo(
    () => createCustomTooltip(questionsMap),
    [questionsMap]
  );
        ...
        <LineChart data={data}>
          <XAxis
            ...
            tickFormatter={(date: Date, index): string =>
              index % 2 === 0
                ? ""
                : `Uke ${getWeekNumber(date)}, ${date.getFullYear()}`
            }
          />
          ...
          {R.range(0, maxQuestions + 1).map((index) => (
            <Line
              ...
              dataKey={`${index}.score`}
              stroke={
                index < 10 ? colorsLineIndex[index] : "rgba(205, 92, 92, 0.5)"
              }
              strokeWidth={2}
              isAnimationActive={false}
          ...
          <Legend
            formatter={(_value, _entry, index) => indexQuestionLookup[index]}
          />
        </LineChart>
      </ResponsiveContainer>
    </div>
...
function createCustomTooltip(
  ...
  return function CustomTooltip({
    ...
    const relevantValues = R.toPairs
      ...
      .filter((tuple) => tuple[0] !== "timestamp") as [
      string,
      { id: string; score: number },
    ][];

    const colors = R.fromPairs.strict(
      payload.map((it): [string, string] => [
        it.dataKey.split(".")[0],
        it.stroke,
      ])
    );
    ...
    return (
      <div className="bg-white border border-border-default rounded p-2">
        <Detail>
          Uke {getWeekNumber(first.payload.timestamp)},{" "}
          {first.payload.timestamp.getFullYear()}
        </Detail>
        {R.pipe(
          relevantValues,
              ...
              <div key={key}>
                <div className="flex gap-2">
                  ...
                  <span style={{ color: colors[key] }}>
                    {questionsMap[value.id]?.question}
                  </span>
